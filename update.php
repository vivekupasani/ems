...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        foreach ($dateFields as $field) {
            $employeeData[$field] = !empty($_POST[$field]) ? formatDate($_POST[$field]) : null;
        }

        // Handle decimal fields
        $decimalFields = ['duty_hours', 'total_hours', 'hours_per_day', 'basic_salary', 'ca', 'da', 'hra', 'ta', 'ma', 'other_allowance'];
        foreach ($decimalFields as $field) {
            $employeeData[$field] = !empty($_POST[$field]) ? formatDecimal($_POST[$field]) : null;
        }

        // Handle gender (capitalize first letter)
        $employeeData['gender'] = !empty($_POST['gender']) ? ucfirst(strtolower($_POST['gender'])) : null;
        ...
        $textFields = [
            ...
            'aadhar_number',
            'salary_category',
            'other_salary_category',
            'salary_pay_band',
            'pf_number'
        ];
        foreach ($textFields as $field) {
            $employeeData[$field] = !empty($_POST[$field]) ? sanitizeInput($_POST[$field]) : null;
        }

        $employeeData['id'] = intval($_POST['id']);
        ...
        $stmt->bind_param(
            "ssssssssssssssssssssssssssssdddssdsddddddddi",
            ...
            $employeeData['other_salary_category'],
    ...
    <div class="bg-gray-50 p-6 rounded-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="pan_number" value="<?php echo htmlspecialchars($employee['pan_number']); ?>"
            ...
            <div>
                ...
                <input type="text" name="aadhar_number"
                    value="<?php echo htmlspecialchars($employee['aadhar_number']); ?>" required pattern="[0-9]{12}"
                    class="mt-1 block w-full p-1 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" />
            </div>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <select name="salary_category" required
                    ...
                    <option value="5th_pay" <?php echo $employee['salary_category'] == '5th Pay' ? 'selected' : ''; ?>>5th
                        Pay</option>
                    <option value="6th_pay" <?php echo $employee['salary_category'] == '6th Pay' ? 'selected' : ''; ?>>6th
                        Pay</option>
                    <option value="other" <?php echo $employee['salary_category'] == 'Any Other' ? 'selected' : ''; ?>>Any
                        Other</option>
                </select>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Duty Hours</label>
                <input type="number" name="duty_hours" value="<?php echo htmlspecialchars($employee['duty_hours']); ?>"
                    required
                    class="mt-1 block w-full p-1 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" />
            </div>
            <div>
            ...
            <div>
                ...
                <input type="date" name="pf_join_date" value="<?php echo formatDate($employee['pf_join_date']); ?>"
                    class="mt-1 block w-full p-1 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" />
            </div>
        </div>
    </div>
