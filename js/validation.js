...
class FormValidator {
    constructor(formId, validationRules) {
        ...
        this.errors = new Map();
        this.toastShown = new Map(); // Track if toast has been shown for each field
    ...
    validateField(input) {
        ...
        if (errorMessage) {
            ...
            // Only show toast if this is the first time we're seeing this error
            ...
            // Reset toast tracking when field becomes valid
            if (hadError) {
    ...
    // Get field label for better toast messages
    getFieldLabel(input) {
        const label = input.parentElement.querySelector('label');
        return label ? label.textContent : input.name.replace(/_/g, ' ');
    }
